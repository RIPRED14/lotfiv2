...
const SampleEntryPage = () => {
  ...
  const handleAddSample = async () => {
    ...
    try {
      ...
      if (pingError) {
        console.error("Erreur lors de la vérification de Supabase:", pingError);
        toast({
          title: "Problème de connexion",
          description: "Impossible de se connecter à la base de données. Vérifiez votre connexion internet ou contactez l'administrateur.",
          variant: "destructive",
          duration: 7000
        });
        
        // Continuer quand même, le mode hors ligne sera utilisé automatiquement
        console.log("Tentative d'ajout en mode hors ligne suite à l'échec de connexion");
      } else {
        console.log(`Connexion à Supabase OK (${endTime - startTime}ms)`);
      }
    } catch (connectionError) {
      console.error("Exception lors de la vérification de connexion:", connectionError);
      // Continuer quand même, le mode hors ligne sera utilisé automatiquement
    }
    ...
    // Informations sur le formulaire global
    ...
    // Vérifier l'état actuel des échantillons
    console.log("Nombre d'échantillons actuels:", samples.length);
    
    // Générer une référence unique pour cet échantillon
    const reference = `REF-${siteVal}-${Math.floor(Math.random() * 100000)}`;
    ...
    // Calculer la DLC par défaut (date actuelle + 21 jours)
    const dlcDate = new Date(currentDate);
    dlcDate.setDate(dlcDate.getDate() + 21);
    const formattedDlc = `${dlcDate.getFullYear()}-${String(dlcDate.getMonth() + 1).padStart(2, '0')}-${String(dlcDate.getDate()).padStart(2, '0')}`;
    
    // Heure actuelle formatée pour ready_time
    const hours = String(currentDate.getHours()).padStart(2, '0');
    const minutes = String(currentDate.getMinutes()).padStart(2, '0');
    const readyTime = `${hours}:${minutes}`;
    
    // Afficher un toast de chargement
    toast({
      title: "Ajout en cours",
      description: "Création d'un nouvel échantillon...",
      duration: 2000
    });
    ...
    try {
      const sampleData = {
        ...
        analysisDelay: delay,
        readingDay: day,
        fabrication: formattedDate,      // S'assurer que la date de fabrication est présente
        dlc: formattedDlc,               // Ajouter DLC avec valeur par défaut
        readyTime: readyTime,            // Ajouter heure de préparation
        brand: brand || '',              // S'assurer que la marque est définie
        reportTitle: reportTitle || `Analyse de ${bact} (${delay})`, // Définir un titre de rapport
        reference: reference             // Ajouter la référence
      };
      
      console.log("Tentative d'ajout d'échantillon avec les données:", { product: defaultProduct, ...sampleData });
      
      // Vérifier l'état de l'utilisateur
      const userInfo = localStorage.getItem('user');
      console.log("Informations utilisateur:", userInfo);
      
      const sampleId = await addSample(defaultProduct, sampleData);
      
      if (sampleId) {
        console.log("SUCCÈS - Échantillon ajouté pour:", bact, "avec ID:", sampleId);
        
        // Déterminer si l'échantillon a été ajouté en mode local ou Supabase
        const isLocalSample = sampleId.toString().startsWith('local-');
        
        // Afficher une confirmation appropriée
        toast({
          title: isLocalSample ? "Échantillon ajouté (mode local)" : "Échantillon ajouté",
          description: isLocalSample 
            ? `Échantillon enregistré localement. Synchronisez plus tard quand la connexion sera rétablie.` 
            : `Échantillon préparé pour l'analyse de ${bact} (${delay}) - Site: ${siteVal}`,
          duration: 5000
        });
      } else {
        console.error("❌ Échec de l'ajout d'échantillon");
        toast({
          title: "Erreur",
          description: "Impossible d'ajouter l'échantillon. Vérifiez votre connexion et réessayez ultérieurement.",
          variant: "destructive",
          duration: 5000
        });
      }
      ...
      // Analyse de l'erreur pour fournir un message plus précis
      let errorMessage = "Un problème est survenu lors de l'ajout de l'échantillon.";
      
      if (error.message?.includes("fetch failed")) {
        errorMessage = "Impossible de se connecter à la base de données. Vérifiez votre connexion internet.";
      } else if (error.message?.includes("timeout")) {
        errorMessage = "La connexion à la base de données a expiré. Le serveur est peut-être surchargé.";
      } else if (error.message?.includes("23502")) {
        errorMessage = "Certains champs obligatoires sont manquants. Veuillez réessayer.";
      } else if (error.message?.includes("23505")) {
        errorMessage = "Un échantillon avec ce numéro existe déjà.";
      }
